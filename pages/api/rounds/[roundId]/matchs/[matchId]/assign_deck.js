  ...
  try {
    ...
    if (data.decks.every(d => !d.playerId)) {
      ...
      data.decks.forEach(deck => {
        const tournamentPlayerDeckIndex = tournamentPlayersDeck.players.findIndex(({id}) => id === deck.playerId);
        const playerIndex = match.player_match_relationships.findIndex(pmr => pmr.player.id === deck.playerId);
