import type { ScoutingProgress, DeckEntry, MatchupEntry } from '@/src/types/tournament';
...
export const RICH_INK_STYLES = Object.fromEntries(
  INK_NAMES.map((ink) => [
    ink,
...
export function deckToRichLabel(deckKey: string): string {
  return deckKey.split('/').map((ink) => `{${ink.toLowerCase()}|}`).join('');
}

export function deckToFrench(deckKey: string): string {
  return deckKey.split('/').map((ink) => FR[ink] ?? ink).join(' / ');
}

export function buildDonutOption(progress: ScoutingProgress) {
...
export function buildBarOption(distribution: DeckEntry[]) {
...
export function buildHeatmapOption(decks: string[], entries: MatchupEntry[]) {
